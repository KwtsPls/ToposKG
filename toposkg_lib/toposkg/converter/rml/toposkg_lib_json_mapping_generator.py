    ...
    def list_pass(self, l : list, key : str, iterator: str):
        ...
        for v in l:
            ...
            if isinstance(v, dict):
                ...
                raise Exception("JSON array: "+key+"contains literal"+v+", mapping generation failed")
            
        if not bool(common):
            raise Exception("JSON array:"+key+" contains JSON objects with no minimal key. Generation failed...")
        else:
            for k in common:
...
generator = JSONMappingGenerator("https://example.org/ontology/","https://example.org/resource/")
generator.generate_default_mapping("/mnt/c/Users/Heyo/Desktop/ResearchTeam/ToposKG-Test-Sets/JSON/generic/bitcoin.json")